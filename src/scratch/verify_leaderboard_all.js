...
async function testFullLeaderboard() {
  ...
  for (const record of borrowRes) {
    ...
    entry.borrowHistory.push({
      bookTitle: record.book_title || book?.title,
      status,
      points: status === 'full_read' ? pts : (status === 'half_read' ? Math.round(pts / 2) : 0)
    });
  ...
  console.log(`Total Scored Readers on Library Leaderboard: ${leaderboard.length}`);
  leaderboard.forEach((r, idx) => {
    console.log(`Rank ${idx + 1}: ${r.name} (${r.className}) -> ${r.points} pts | ${r.fullRead}f • ${r.halfRead}h | ${r.booksRead} books`);
  });
}
