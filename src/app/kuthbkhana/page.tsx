...
function parseRemarks(remarks: string = '') {
  let type: 'Muthala' | 'Point picking' = 'Muthala';
  let clean = remarks.trim();

  if (/^\[Point [pP]icking\]/i.test(clean)) {
    type = 'Point picking';
    clean = clean.replace(/^\[Point [pP]icking\]\s*/i, '');
  } else if (/^\[Muthala\]/i.test(clean)) {
    type = 'Muthala';
    clean = clean.replace(/^\[Muthala\]\s*/i, '');
  }

  let kithabName = clean;
  let notes = '';

  const dashMatch = clean.match(/^(.+?)\s*-\s*(.+)$/);
  if (dashMatch) {
    kithabName = dashMatch[1].trim();
    notes = dashMatch[2].trim();
  }

  return {
    type,
    title: kithabName || clean,
    notes,
    raw: clean,
  };
}
                      ...
                      >
                        ...
                        <td className="py-3 px-4">
                          <div className="flex items-center space-x-2 flex-wrap gap-y-1">
                            ...
                            {parsed.notes && (
                              <span className="text-[11px] text-slate-500 font-medium">({parsed.notes})</span>
                            )}
                          </div>
                        </td>
