    ...
    try {
      ...
      const res = await fetch('/api/scores/bulk-upload', {
        ...
        body: JSON.stringify({
          ...
          festivalName: formData.programName,
            ...
            <table className="w-full text-left text-xs">
              <thead className="bg-slate-100/70 text-slate-500 uppercase tracking-wider font-semibold sticky top-0">
                <tr>
                  ...
                  <th className="py-2.5 px-4">Program / Activity</th>
              ...
              <tbody className="divide-y divide-slate-100">
                {loading ? (
                  ...
                  recentRecords.map((r: any) => {
                    ...
                    const progName = r.competition?.program?.name || (r.remarks && r.remarks.includes('-') ? r.remarks.split('-')[0].trim() : 'Program / Event');
                    const compName = r.competition?.name || (r.remarks && r.remarks.includes('-') ? r.remarks.split('-')[1]?.split('.')[0]?.trim() : r.remarks) || 'Activity';

                    return (
                      <tr key={r.id} className={`hover:bg-slate-50 transition-colors ${isSelected ? 'bg-amber-50/50' : ''}`}>
                        ...
                        <td className="py-2.5 px-4">
                          <div className="space-y-1">
                            <div className="flex flex-wrap items-center gap-1.5">
                              <span className="px-2 py-0.5 rounded-md text-[10px] font-extrabold border bg-amber-100 text-amber-900 border-amber-300 inline-flex items-center space-x-1">
                        ...
                        <td className="py-2.5 px-4 text-right">
                          <div className="space-y-0.5">
                            <div className="font-mono font-black text-amber-900 text-xs sm:text-sm">
          ...
          <div className="bg-white rounded-2xl max-w-lg w-full p-6 shadow-2xl border border-slate-200">
            ...
            <form onSubmit={handleSaveSingleScore} className="mt-4 space-y-3.5">
              ...
              {(() => {
                ...
                return (
                  <div className="p-3 bg-gradient-to-r from-amber-50 via-amber-100/40 to-emerald-50 rounded-xl border border-amber-200/90 shadow-2xs space-y-1">
          ...
          <div className="bg-white rounded-2xl max-w-md w-full p-6 shadow-2xl border border-slate-200">
            ...
            <form onSubmit={handleSaveEdit} className="mt-4 space-y-3.5">
              ...
              {(() => {
                ...
                return (
                  <div className="p-2.5 bg-gradient-to-r from-amber-50 to-emerald-50 rounded-xl border border-amber-200 text-[10px] space-y-0.5">
                    <div className="flex items-center justify-between font-bold text-slate-800">
                      <span>Calculated Result Preview:</span>
                      <span className="font-mono text-amber-900 font-black text-xs">+{formatPoints(editCalculatedFinalPoints)} pts</span>
                    </div>
                    <div className="text-slate-600">
                      {editScoreVal} × {editPrizeMult}x ({editFormData.position || 'Standard'}) {editLevelObj ? `× ${editLevelMult}x (${editLevelObj.name})` : ''} = <strong className="text-amber-800 font-bold">{editCalculatedFinalPoints} pts</strong>
                    </div>
                  </div>
                );
              })()}
